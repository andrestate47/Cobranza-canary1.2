...
export async function GET(request: NextRequest) {
  try {
    ...
    // Si no es ADMINISTRADOR, filtrar por ruta
    if (user.role !== "ADMINISTRADOR") {
      // Si el usuario no tiene ruta asignada, no verá ningún cliente
      if (!user.rutaId) {
        whereClause.rutaId = "sin-ruta-imposible" // Forzar a no encontrar nada
      } else {
        whereClause.rutaId = user.rutaId
      }
    }
    // Si es ADMINISTRADOR, verá todos los clientes (no agregamos filtro de ruta)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { codigoCliente, documento, nombre, apellido, direccionCliente, direccionCobro, telefono, referenciasPersonales, pais, ciudad, ubicacion, mapLink, numeroRuta, rutaId } = body
    ...
    const ubicacionLimpia = ubicacion ? String(ubicacion).trim() : null
    const mapLinkLimpio = mapLink ? String(mapLink).trim() : null
    ...
    const nuevoCliente = await prisma.cliente.create({
      data: {
        ...
        mapLink: mapLinkLimpio,
        numeroRuta: numeroRuta?.trim() || null,
    ...
    return NextResponse.json({
      ...
      mapLink: nuevoCliente.mapLink,
      foto: nuevoCliente.foto,
      fotoDocumento: nuevoCliente.fotoDocumento,
      activo: nuevoCliente.activo,
      numeroRuta: nuevoCliente.numeroRuta,
      rutaId: nuevoCliente.rutaId
    })
  } catch (error) {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json()
    const { id, codigoCliente, documento, nombre, apellido, direccionCliente, direccionCobro, telefono, referenciasPersonales, pais, ciudad, ubicacion, mapLink, numeroRuta, rutaId } = body
    ...
    return NextResponse.json({
      ...
      mapLink: clienteActualizado.mapLink,
      foto: clienteActualizado.foto,
      fotoDocumento: clienteActualizado.fotoDocumento,
      activo: clienteActualizado.activo,
      numeroRuta: clienteActualizado.numeroRuta,
      rutaId: clienteActualizado.rutaId
    })
  } catch (error) {
